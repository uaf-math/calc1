...
\addplot[domain=-4.85:5.4, <->, ultra thick] {-(x+2)^2*(x-4)/20};
...
function information & \quad \hspace{1.5cm} what you conclude about the behavior of $f$\hspace{1.5cm} \quad\\ \hline \hline
 ...
 $f(0) =1$ & \\ \hline
 $f'(-3) =  0$ & \\ \hline
% $f'(x) < 0 $ on $(-\infty, -5), (1,4), (7, \infty)$ \\ \hline
% $f'(x) > 0$ on $(-5, 1), (4,7)$ \\ \hline
...
\item (12 points) Evaluate the limits below. Indicate a use of L'H\^{o}pital's Rule by a symbol (such as \textbf{L'H} or \textbf{H}) over the equal sign. 
	\begin{enumerate}
	\item $\d{\lim_{t \to 4} \frac{\sin(\pi t)}{t^2-16} }$
	\vfill
	\item $\d{\lim_{x \to 0 } \frac{x-4\cos(\pi x)}{3e^x} }$
	\vfill
	\item $\d{\lim_{x \to \infty} x(5-5e^{\frac{1}{x}})}$
	\vfill
	\end{enumerate}
\newpage
%%%Optimization
