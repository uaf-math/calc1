...
\draw[<->] (0,-3.2) -- (0,2.2); %node[left] {$A(x)$};
\draw (-.5, 1.5) node{$A(x)$};
\draw[help lines, dashed] (-3,-3) grid (6,2);
\draw[style= ultra thick,<-] (-3.2,-3.2) -- (0,0);
\draw[->,ultra thick] (2,0)-- (3,-1)-- (4, -1) --(6.1,1.1);
\draw[style= ultra thick] (0,0) arc (180:0:1);
\foreach \x in {-3,-2,-1,0,1,2,3,4,5,6}
\draw (\x,-3.1) node[below] {$\x$};
\foreach \y in {-2,-1,0,1,2}
\draw (-3.1,\y) node[left] {$\y$};
\node[fill=white, minimum size=1pt,circle,draw,scale=.55] at (-2,-2){};
\node[fill=black, minimum size=1pt,circle,draw,scale=.5] at (-2,2){};
\end{tikzpicture}
\end{center}
...
\item For $H(x)$ from part \textbf{f.}, what is the value of $H'(1)$.
