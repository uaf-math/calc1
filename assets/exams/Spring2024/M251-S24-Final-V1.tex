...
\begin{subproblems}
	\item  $\displaystyle{ \int \sqrt[4]{x^{3}} + \sqrt{2} - \sin(x)\: dx }$
	\vfill
	\item $\displaystyle{ \int \frac{2x \ln(x^2 +1)}{ x^2 +1}\: dx  }$
	\vfill
	\item $\displaystyle \int%_{0}^{2} 
	\frac{x^2}{\sqrt{1+x^3}} \: dx$
	\vfill
...
A giant spherical snowball is melting. Its volume is decreasing at a rate of $1$ cubic meter per hour. \emph{How fast} is the radius of the snowball decreasing when its radius is $2$ meters? (The volume $V$ of a sphere of radius $r$ is given by $\displaystyle V=\frac{4}{3}\pi r^3$.)
...
\begin{subproblems}
\item $\ds{\lim_{ x\to \infty} \frac{\ln(2x^4 +3)}{x^4}}$ 
\vfill
...
\begin{subproblems}
	...
	\item  $\displaystyle{ f(\theta) = \ln ( \sec \theta  + \cot \theta )}$
\vfill
	\item   $\ds g(x)=\frac{\sqrt{3}}{4}+\frac{\sqrt{x}}{5}-\frac{5}{\sqrt{x}}$
\vfill


\item  $\ds h(x)=\frac{e^{3x}}{\sin(x)}$
\vfill
...
\begin{center}
 \begin{tikzpicture}
\begin{axis}[scale=1,xscale=2, thick, my style,xmin=-3, xmax=9, ymin=-10.5, ymax=9, xtick={-3, ..., 9},
mark size=3.0pt, ]
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[scale=1,xscale=2, thick, my style,xmin=-3, xmax=9, ymin=-10.5, ymax=9, ytick=\empty,xtick={-3, ..., 9},
mark size=3.0pt, ]
%\addplot[ultra thick, ->,domain=-4:9, samples=100]{(0.1)*(0.25*x*x*x*x-2*x*x*x+x*x)+4};
%\addplot[]{x};
%\addplot[ultra thick, <-,domain=-6:0, samples=100]{-x};
