...
\begin{subproblems}
	\item $\displaystyle \lim_{t \rightarrow 2} \frac{e^{t-2}-t+1}{\, t^2-4t+4 \,}$
	\vfill
	\item $\displaystyle \lim_{x \rightarrow \infty} \frac{3x-x^4}{\, 3x^4-4x^3-1 \,}$
	\vfill
	\item $\displaystyle \lim_{\theta \rightarrow 0} \frac{4\sin(\theta) - 4}{1-\theta -e^{\cos(\theta)}}$
	\vfill
\end{subproblems}
...
\def\xx{2}
\def\off{.3}
\def\yy{7} %%%%%% change this for a new problem %%%%%%%%
%Max/min problem
...
\hfill \begin{tikzpicture}[scale=.8]
...
\problem{11 points} Consider the function $\ds{g(x) = \frac{x^{2/3}}{x-3}.}$  After  simplification, $\ds{ g'(x) = \frac{-(x+6)}{3x^{1/3}(x-3)^{2}}.}$
...
\problem{12} \emph{Sketch} a graph of a function $f(x)$ that satisfies all of the following properties.
...
\item $f(3) =4$ and $f'(3)=0$
\item $\ds{\lim_{x \to -1^+} f(x) = -\infty}$
\columnbreak
\item $f'(x) > 0$ on $(-\infty,-1)\cup(-1,3)$
\item $f'(x) < 0$ on $(3,\infty)$
\item $f''(x) > 0$ on $(-\infty, -1) \cup (5,\infty)$
\item $f''(x) <0$ on $(-1,5)$
\end{itemize}
\end{multicols}
...
xmin=-7, xmax=12.3, ymin=-4, ymax=10, minor y tick num=1,
...
%(4,2)%
%(6,2.1)};
\addplot[<->, domain=-.5:7, smooth, variable=\x, ultra thick] plot ({\x}, {6*\x/(\x+2)});
%\addplot[->, domain=5:12, smooth, variable=\x, ultra thick] plot ({\x}, {3*(-0.75)*rad(atan(\x-5))});

%\draw[smooth, ultra thick] (-5,4) to[out = -90, in = 180-60] (-4,0)  parabola bend (-2,-2) (-1,0);
\path (2,4.5) node {{\Large $f(x)$}};
\end{axis}
\end{tikzpicture}
...
\begin{subproblems} 
%\begin{enumerate}
	\item $\displaystyle \int \left(4x^4 + \sin(x) - e^x + \sqrt{2}\right)\ dx$
	\vfill
	\item $\displaystyle \int \frac{1+x^\frac{2}{3}+x^3}{x}\ dx$
	\vfill
%\end{enumerate}
\end{subproblems}
