...
\addplot[ultra thick,>-] (-6, -2) parabola bend (-3,6)  (0,0) to  (4,-2) to (6,-2);
\addplot[ultra thick,->] (6, 8) parabola bend (9,3)  (11.5,10);
\node at (-4,8){\large{$f(x)$}};
\draw[fill=white, thick] (6,-2) circle  (1.2 mm);
\draw[fill=black, thick] (6,8) circle  (1.2 mm);
\end{axis}
...
\item $\displaystyle{ \lim_{x \to -3} f(x) }= \ans$
\bigskip
...
\begin{center}
 \begin{tikzpicture}
\begin{axis}[scale=2,xscale=1, thick, my style, xtick={-6,-5,...,10}, ytick={-6,-5,...,6},xmin=-6, xmax=11, ymin=-6, ymax=6, 
mark size=3.0pt, grid = major,yticklabels={,,} , xticklabels={,,}]
\addplot[ultra thick, ->,domain=0:10, samples=100]{100*x/(x*x*x+50)};
