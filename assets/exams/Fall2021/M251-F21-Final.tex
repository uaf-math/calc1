...
\subproblem{} $\displaystyle f(x) = (\cos x)( \ln(x^2 + 1))$\\

$f'(x)=$\\
\vfill
\subproblem{}  $\displaystyle g(x) = e^{\sqrt{x}}+5x^3+\sin\left(\frac{\pi}{4}\right)$\\
...
\subproblem{} $\displaystyle \int \left(2x^4-\frac{4}{x^2}\right) dx$
...
\subproblem{} $\displaystyle \lim_{x \rightarrow 6} \frac{\frac{1}{6}-\frac{1}{x}}{x-36}.$

\vspace{1in}
\subproblem{} $\displaystyle \lim_{x \rightarrow 3^-} \frac{2x^2-3x}{x^2-7x+12}$
...
\problem{10 points} A drone is launched off a 2-foot platform. Its upward velocity in feet per second at $t$ seconds is measured by the function $v(t)=\frac{3}{1+t^2}$. 
...
\problem{10 points} A manufacturer discovers that the revenue gained by producing and selling $x$ products is $R(x)=420x$ and the cost is $C(x)=2x^2+5000$.
...
\begin{itemize}
\item $G'(x)$ is negative for all x in the domain  $(-\infty,0) \cup (0,\infty).$
\item $G''(x)$ is negative in the interval $(-2,0)$
\item $G''(x)$ is positive in the interval $(-\infty,-2) \cup (0, \infty).$
\item $\lim_{x\to\infty} G(x)=1$.
\item $\lim_{x\to 0^+} G(x)=\infty$ and $\lim_{x\to 0^-} G(x)=-\infty$
\end{itemize}
...
\draw[ultra thick, <-] plot [smooth] coordinates {(-2,-2)  (-1,1) (0,2)}; 
\node[shape=circle, ultra thick, inner sep=1pt, draw,fill=white,minimum size=3mm] at (0,2){};
\node[shape=circle, ultra thick, inner sep=1pt, draw,fill=white,minimum size=3mm] at (0,3){};
\node[shape=circle, ultra thick, inner sep=1pt, draw,fill=black,minimum size=3mm] at (0,4){};
\draw[ultra thick] (0,3) -- (2,5);
...
\bigskip
\subproblem{} Find $\displaystyle \lim_{x \to 0^-} Q(x)$
\vskip 0.75cm
...
%\node at (-.1,-.15){$0$};
\draw[style= ultra thick] (0,2) --  (2,-2)--(3,-2)--(7,2);
%\draw[style= ultra thick] (1,0) arc (0:180:1);
%\draw[style=ultra thick] (1,0) --(4,-1)--(5,0);
...
\bigskip
\subproblem{} What is the value of $f(1)$?
\vskip 0.75cm
...
\subproblem{} What is the value of $H'(4)$?
\vfill
...
\problem{12 points}
Water flows into a tank at a rate of $r(t)=6+4t-t^2$ liters per minute from $t=0$ to $t=5$ minutes.
...
\subproblem{}  At time $t=0$, the tank contained 50 liters of water.  How much water is in the tank at time $t=2$?
\vspace{.8in}
