...
\subproblem{} $\displaystyle f(x) = \sqrt{x} e^{2x}$\\


\vfill
\subproblem{}  $\displaystyle g(x) = \left(\ln(x)+\frac{3x}{4}\right)^5$\\
...
\problem{8 points} Evaluate the definite integrals below. \textbf{Simplify} your final answers.
	\subproblem{} $ \displaystyle \int_1^3 4x - 5 \: dx$
	\vfill
	\subproblem{} $ \displaystyle \int 3(\sin x)^4 \cos x \: dx $
	\vfill
...
\draw[help lines, dashed] (-4.2,-3.2) grid (7.2,5.2);
\draw[->] (-4.2,0) -- (7.2,0) node[right] {$x$};
\draw[->] (0,-3.2) -- (0,5.2) node[above] {$y$};
\draw  (-3.5,4.5) node[right] {\large{$f(x)$}};
\draw[ultra thick, <-] plot [smooth,domain=-4.1:0] (\x,{(1.25)*(\x+2)*(\x+2)-1}); 
...
\subproblem{} What is the value of $G(4)$?
\vfill
