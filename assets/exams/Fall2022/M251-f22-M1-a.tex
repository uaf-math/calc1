...
\item (12 points)
	...
	\vspace{1in}
	\item Find the derivative of $f(x) = \sqrt{6-x}$  using the limit definition of the derivative. No credit will be awarded for using other methods.
	\end{enumerate}
\newpage
...
\begin{enumerate}
\begin{multicols}{2}
\item $\displaystyle{\lim_{x\to-4^-} g(x) = }$
\vfill
\item $\displaystyle{\lim_{x\to-4^+} g(x) = }$
\vfill
\item $\displaystyle{g(-4)=}$
\vfill
\item $\displaystyle{\lim_{x\to 2} g(x) = }$
\vfill
\end{multicols}
