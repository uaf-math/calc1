...
%%%%page 1 problems
\item $\ds f(\theta) = \sin(3\theta^5+2\theta +1) $\\

\vfill 

\item $\ds p(x)=\frac{3}{\sqrt{2x}}+\left( \frac{x+8}{3} \right)^2 $\\
...
\item $\ds f(t)=\sqrt{t+ \cos^3(t)}$\\
%From V1
\vfill

\item $\ds f(x)= x^{5/3}\sec(x)$\\
\vfill
\newpage
%%%%%page 3 problems


\item $\ds f(x)=\frac{x}{x+\tan(x)} $\\
%from V1
\vfill

\item $\ds g(x)=(\sin(\ln(x)))^6$\\

\vfill

\item $\ds f(x)=e^{5x}(2-x)$\\
...
\vfill 
\newpage
\item $\ds k(x)=\frac{x^2 \ln(x) +5}{x}$\\
%from V1
\vfill

\item $\ds f(x)=x^p+ \ln(ax+3)$ (Assume $p$ and $a$ are fixed positive constants.)
\vfill

\item Find $\ds \frac{dy}{dx}$ for  $\quad \ds x+y + \pi = ye^x$\\
\vfill
