...
\doctitle{Math F251X: Derivative Proficiency Retake 2}
\docdate{October 31, 2025}
\doccourse{UAF Calculus I}
\docversion{v-1 try 3}
...
\begin{subproblems}
\item $\ds f(t) = e^t(3 - t^4)$ \vfill
\item $\ds  r(\theta) = \tan\left(\sqrt3 + \theta^2\right)$ \vfill
\item $\ds g(z) = (3z - 4)(z^2 + 7)$\vfill
\newpage
\item $\ds f(x) = 3\cos(x) + x\sqrt{x + 1}$\vfill
\item $\ds f(r) = \frac{r^3 + \sqrt{r} - 2}{r}$\vfill
\item $\ds G(x) = \left(\frac{x - \ln(4)}{2}\right)^3$ \vfill

\newpage
\item $\ds f(y) =  e + \cos(y^\pi)$ \vfill
\item $\ds f(x) = \frac{2\sec(bx)}{3x^3}$ (where $b$ is a constant)\vfill
\item $\ds y = x^{1/4}e^{-\sin(x)}$\vfill
\newpage
\item $\ds y(t) = \ln(2t + \sin(t^2))$ \vfill
\item $\ds g(x) = \arctan(e^{x})$ \vfill
\item Compute $\frac{dy}{dx}$ if\quad $\ln y - 5x = x^2y$. You \fbox{must} solve for $\frac{dy}{dx}$.
\vspace{3.5in}
\end{subproblems}
\end{document}