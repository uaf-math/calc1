...
\item   $\ds f(x)=\frac{x^3}{4} + \frac{2}{\sqrt{x}} + \sqrt{50}$
\vfill

%base other than e
%product rule inside of chain rule
\item   $\ds f(x)=\log_b(x^2\sin x)$ (where $b > 1$)
\vfill

%double chain
%cosine
\item   $\ds f(x) = \left(e^x + \cos(2x)\right)^{5/4}$
\vfill
...
%chain rule
%understanding of constants 
\item   $\ds y=\pi \left( \frac{x+2}{2} \right)^3$
...
%thoughtful algebra
%quotient/product
\item   $\ds f(x) = \frac{8+x^2}{x\cos(\pi)}$
\vfill

\newpage
%chain inside of product
%decimal exponent
\item   $\ds f(x) = x\ln(5+ \frac{x}{5})$
\vfill

\item  $\ds f(x) = e^{-x} + e^2 + x^{0.8}$
\vfill

\item  Find $\ds \frac{dy}{dx}$ for $\ds x^2+y^2=25+2xy^2$. You must solve for $\ds \frac{dy}{dx}$.
\vfill
