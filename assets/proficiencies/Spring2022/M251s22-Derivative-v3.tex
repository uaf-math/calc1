...
\newpage
\item   $\ds f(x)=\tan^{-1}(x^2)$
\vfill
\item   $\ds f(x)=(\sin(x)+x^{-2.3})^5$
\vfill



\item   $\ds f(x) = \frac{3}{\sin(x)}$
\vfill

\newpage
\item   $\ds y=e^{-x}\cos\left(\frac{x}{2}\right)$
\vfill

\item   $\ds y=\ln\left(\sqrt{x^{6}-x}\right)$
\vfill

\item   $\ds f(x) = \frac{e^x}{(x^2+2)^3}$
\vfill
...
\item   $\ds f(x) = \tan \left( x^2 - e^{4x}\right)$
\vfill

\item  $\ds f(x) =\frac{x+2\sin(x)}{\sin(8)}$
\vfill

\item  Find $\ds \frac{dy}{dx}$ for $\ds x^3-y^4=ye^x$. You must solve for $\ds \frac{dy}{dx}$.
\vfill
