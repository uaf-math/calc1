...
\item   $\ds f(x)=\frac{\sqrt{x}}{6}+\frac{5}{\sqrt{x}}-\frac{4}{\sqrt{5}}$
\vfill
...
\newpage
\item   $\ds f(x)=(x+3^x+e^3)^5$
\vfill

\item   $\ds f(x)=4 \sin^{-1}(4x)$
\vfill
...
\newpage
\item   $\ds y=\left( x^{0.1}+1\right)^{-2/5}$
\vfill

\item   $\ds y=x^4e^{4x}+e^{-x}$
\vfill

\item   $\ds f(x) = \frac{\sin(\pi/x)}{x^2+2}$
\vfill
...
\item  $\ds f(x) = \ln\left(\frac{\sin^2(2x)}{2x+1} \right)$
\vfill

\item  Find $\ds \frac{dy}{dx}$ for $\ds xe^y+5(x^3+y^3)=0$. You must solve for $\ds \frac{dy}{dx}$.
\vfill
