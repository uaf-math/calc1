...
\item $\ds \int \left(\frac{-4}{x^5}+\cos(x)\right) \; dx$
\vfill
%easy u-sub 
%inside sine

\item $\ds \int e^{12x}\; dx$
\vfill

%%Def integral + substitution
\item $\ds \int_0^{\sqrt{\pi}} x\sin({x^2})\; dx$
\vfill


\newpage
% u-sub for ln
\item $\ds \int \frac{1+\sec^{2}(3x)}{3x+\tan(3x)}\; dx$
\vfill

%basic rules: power, ln, contants
\item $\ds \int \left( \frac{x^{2}}{3}+\frac{5}{x} -\frac{1}{2}\right)\; dx$
\vfill

%thoughtful u-sub, power rule, 
\item $\ds \int \frac{6}{2x(\ln x)^3}\; dx$
\vfill
...
\item $\ds \int \frac{\arcsin(x)}{\sqrt{1-x^2}}\; dx$ $\quad$ (recall $ \arcsin(x)=\sin^{-1}(x) $)
\vfill

%no sub + u-sub
%e and trig
\item $\ds \int (e^{-5x}+\cos(3x)) \; dx$
\vfill

\newpage
%nonproduct rule
\item $\ds \int_{-1}^1 x(5-x)\; dx$
\vfill
%sqrt in denominator, u-sub
\item $\ds \int \frac{x^2}{\sqrt{1-x^3}} \; dx$
\vfill
%sophisticated substitution \# 283
\item $\ds \int \frac{t}{t+3} \: dt$
\vfill
