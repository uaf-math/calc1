...
\item   $\ds f(x)=\frac{\cos (x/2)}{x^6}$
\vfill

\item   $\ds f(x)=\frac{1}{9x}+\sqrt{5 - x}$
\vfill

\item   $\ds f(\theta)=\ln(\sec \theta + \tan \theta)$
\vfill

\newpage
%%%page 3
\item   $\ds f(q) = \frac{q \ln(q)}{\ln 2}$
\vfill

\item   $\ds f(x)=\frac{\cos(x)}{\sin(x)}$
\vfill

\item   $\ds y=\pi \left( \frac{6+x}{12} \right)^5$
\vfill
...
\newpage
%%%% page 4
\item   $\ds f(x) = \left( \sin(x^3+e^3)\right)^5$
\vfill

\item  $\ds f(x)= \arctan{(3x)}$ $\quad$ (this is the same as writing $\ds f(x)=\tan^{-1}{(3x)}$)
\vfill

\item  Find $\ds \frac{dy}{dx}$ for $\ds 2y+x=y\sin(x)$. You must solve for $\ds \frac{dy}{dx}$.
\vfill
