...
\vfill 
\item $\ds g(x)=\ln(\sec(x) + \tan(x))$
%\textcolor{red}{type: straight chain rule with LN, secant, tangent}

\vfill 
\item $\ds h(\theta)=\frac{\sin(\theta)}{\theta^3}$
%\textcolor{red}{type: straight product or quotient rule}
\vfill 
\newpage
%page 2 problems
\item $\ds y=(\cos(4x)+e^{x})^3$
%\textcolor{red}{type: double chain rule}

\vfill 
\item $\ds k(x)=\arctan(x^2)$
%\textcolor{red}{type: arc trig function with chain rule}

\vfill 
\item $\ds r(t)=\frac{t^3-5t^2+t^{1/3}}{t} $
...
\newpage
%page 3 problems
\item $\ds f(x)=\sqrt{1+x^a}$ where $a$ is a fixed constant
%\textcolor{red}{type: simple chain with parameter}
\vfill 
\item $\ds y=\ln \left( \frac{x}{1+2x}\right)$
%\textcolor{red}{type: Chain with quotient}

\vfill 
\item $\ds y=\sin^5(x+e^{-x}) $
