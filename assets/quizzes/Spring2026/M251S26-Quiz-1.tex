...
\begin{document}
\textbf{Please circle your instructor's name:} \hfill James Gossell  \hfill Gordon Williams \\
...
%Problem
\problem{7} Determine the following for the function $f(x) = 2x^2 +5x - 3$. \textbf{Simplify} your answers.
\begin{subproblems}
\item $f(-1)$ \\
  \vfill
...
\item $f(z-2)$ \\
  \vfill
  \quad \hspace{4in} \underline{\hspace{2in}}
\item Find all values of $x$ such that $f(x) = 0$ \\
  \vfill
  \quad \hspace{4in} \underline{\hspace{2in}}
\end{subproblems}
...
\begin{subproblems}
\item The line containing the point $(3,5)$ with slope
  $\frac{1}{3}$. \\
  \vfill
  \quad \hspace{4in} \underline{\hspace{2in}}
\item The line containing the points $(3,5)$ and $(-2,8)$. \\
  \vfill
  \quad \hspace{4in} \underline{\hspace{2in}}
\end{subproblems}

\problem{2} In the right triangle below, suppose $a=2$ and $b=3$. Determine the exact value of $\cos(A)$.
...
\begin{center}
			...
			\addplot[excl,thick] coordinates {(-1,4)};
...
\item For which $x$-value does $G(x)=1$? \hfill \underline{\hspace{2in}} \\
  \vfill
\item Graph the transformed function $G(x+1)-2$ on the axes above. \\
  \vfill
\end{subproblems}
