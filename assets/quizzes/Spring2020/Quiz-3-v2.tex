...
\begin{subproblems}
\item  $\ds \lim_{x\to -3}\frac{x^2+2x-3}{x^2+5x+6}$
\vfill

\item  $\ds \lim_{x\to 4} \:\frac{2-\sqrt{x}}{4x-x^2}$
\vfill

\item $\ds \lim_{h\to 0^-}\frac{2h^2+14h}{|h|}$
\vfill


\end{subproblems}
\problem{4} Use the Intermediate Value Theorem to \textbf{show} that the equation $e^x=6-8x$ has a root in the interval $(0,1).$
\vfill
...
$\ds
f(x)=\begin{cases} 2-2x & x < 1\\ 3 & x=1\\
\sqrt{x-1} & x>1.
\end{cases}
$
\begin{subproblems}
...
\noindent  $\d{\lim_{x \to 1} f(x)=}$\\
\vspace{1in}
\item Is $f$ continuous at $x=1$? Explain using the definition of continuity.
\vspace{1in}
\end{subproblems}
\vfill
...
\draw[thick,->] (-2,0) -- (4,0); \draw[thick,->] (0,-2) -- (0,4);
\node at (3.5,-.2){$x$}; \node at (-.2,3.5){$y$};
\draw[ultra thick, smooth, <->] plot coordinates {(-2,-1)(1,3)(2,2)(4,-2)};
\node[fill=white,inner sep=1.5pt] at (2,-.5){2};
