...
\doctitle{Math 251: Quiz 3 MAKE UP}
\docdate{Spring 2022}
...
%%points solid
\addplot[mark=*,only marks] coordinates {(3,2)(5,5) (0,1.5)};
%%points open
\addplot[mark=*,fill=white,only marks] coordinates {(0,4.5)(5,3)};
%%Curves
\addplot[ultra thick, smooth,<-] coordinates {(-3,4) (-2,3.5) (0,1.5)};
\addplot[ultra thick, smooth] coordinates {(0,4.5) (3,2)};
...
\begin{subproblems}
\item $\d{\lim_{ x\to 4} \frac{x^2-3x-4}{x^2-16}}$ 
\vfill
\item $\d{\lim_{ x\to 1} \frac{2-\frac{2}{x}}{x-1}}$ 
\vfill
\item $\d{\lim_{ x\to 2^-} \frac{5x}{x-2}}$ 
\vfill
\item Given $\d{\lim_{ x \to 5} f(x) = 10}$ and $\d{\lim_{ x \to 5} g(x) = -1}$, evaluate $\displaystyle{\lim_{ x \to 5}\; 2\left(\frac{x+1}{f(x)+g(x)}\right)}.$ 
\vspace{.5in}
\end{subproblems}

\problem{4} Use the Intermediate Value Theorem to show that the polynomial $p(x)=x^4-5x+1$ must reach a $y$-value of $-1$ for some $x$-value on the interval $[0,1].$ \\
\vfill
\end{document}