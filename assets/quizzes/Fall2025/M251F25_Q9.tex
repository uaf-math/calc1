...
\problem{10}You are asked to contruct a right circular cylinder with radius $r$ and height $h$ such that the sum of the radius and the height is constant and equal to 6 inches, i.e.
\[
r + h = 6.
\]
Determine the dimensions of such a cylinder that produce the \textbf{maximum volume}. Note that the volume $V$ of a cylinder is given by
\[
V = \pi r^2 h.
\]
...
\begin{subproblems}
	...
	\vfill
	\item $\displaystyle \lim_{x \rightarrow 0^+}\, 2x\,\ln(3x)$
	\vfill
\end{subproblems}
\vfill

\problem{7} Evaluate the following indefinite integrals (antiderivatives):
\begin{subproblems}
	\item $\displaystyle \int \left(\sqrt{x}+4e^x-\sin(x)\right)\, dx$
	\vfill
	\item $\displaystyle \int \frac{2x^5-x^2+3}{x^2}\, dx$
	\vfill
\end{subproblems}
\vfill
