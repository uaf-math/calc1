...
\addplot[domain=0:2,ultra thick]{(1-6)/(0-2)*(x-0)+1};
\addplot[domain=2:4, ultra thick] {2*(x-2)*(x-4)+6};
%\addplot[domain=4:6, ultra thick] {2.+(x-4)^{2}};
\addplot[domain=4:6, ultra thick] {6-.75*(x-4)^2};
\addplot[domain=6:8, ultra thick] {(3-5)/(6-8)*(x-6)+3};
\addplot[mark=*,only marks] coordinates {(0,1)(8,5)(6,4.5)};
\addplot[mark=*,fill=white,only marks] coordinates {(6,3)};
\end{axis}
\end{tikzpicture}
\end{adjustbox}
...
shown on the graph below, on the interval $[0,2]$. It has the property that $f(2) = 0$ and $f(0) = \frac{3}{2}$.
...
\addplot[domain=0:2, ultra thick]{-(3/4*((x-1)*(x-1)*(x-1)))+.75};
%\addplot[domain=0:2, dashed]{( ((3-1)*(3-1)*(3-1)+1) - ((0-1)*(0-1)*(0-1)+1))/(3-0 )(x-0)+(0) };
\end{axis}

\end{tikzpicture}

 Estimated value(s)  of $c$ (to the nearest tenth) predicted by MVT (list all): \ans[3]
 \end{subproblems}
 \end{adjustbox}
%\end{center}
